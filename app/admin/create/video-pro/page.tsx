...
import { Plus, Trash2, ArrowLeft, Film, Check, Upload, Loader2, Play } from 'lucide-react';
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-2xl border border-white/10">
      <div
        ...
        style={{ background: bgStyle.gradient }}
      >
        ...
        <p className="relative text-white/60 text-[10px] tracking-[0.3em] uppercase mb-3">Βίντεο Πρόσκληση</p>
        <h2 className="relative text-white text-3xl italic leading-tight"
          style={{ fontFamily: `'${effectiveFont}', serif` }}>
          {brideName || 'Νύφη'}
        </h2>
        <div className="relative my-1.5 text-xl" style={{ color: palette.primary }}>&</div>
        <h2 className="relative text-white text-3xl italic leading-tight"
          style={{ fontFamily: `'${effectiveFont}', serif` }}>
          {groomName || 'Γαμπρός'}
        </h2>
        <div className="relative mt-4 w-20 h-px" style={{ backgroundColor: palette.primary, opacity: 0.8 }} />
        <p className="relative mt-3 text-white/80 text-[10px] tracking-widest uppercase">{dateStr}</p>
        <div className="relative mt-5 w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center">
          <Play size={16} className="text-white ml-1" fill="white" />
        </div>
      </div>
      <div className="bg-[#fdfaf6] py-3 px-6 text-center">
        <p className="text-[10px] text-[#5c3320]/50 uppercase tracking-widest">Video Pro Preview</p>
      </div>
    </div>
  );
}
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex gap-8 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {FONTS.map((f) => (
                    ...
                    >
                      ...
                      <span className="text-white text-2xl leading-tight mb-1" style={{ fontFamily: `'${f.name}', serif` }}>
                        Νύφη & Γαμπρός
                      </span>
            ...
            {/* Video section */}
            <section className={sec}>
              <SecTitle title="Βίντεο & Εικόνα" desc="Το κεντρικό βίντεο, εξώφυλλο και μουσική" />
              <div>
            ...
            <section className={sec}>
              <div className="flex items-center justify-between">
                <SecTitle title="Εκδηλώσεις" desc="Μυστήριο και δεξίωση" />
