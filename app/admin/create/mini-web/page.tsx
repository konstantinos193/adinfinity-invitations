...
import { Plus, Trash2, ArrowLeft, Globe, Check, Upload, Loader2 } from 'lucide-react';
...
/* ── Font catalogue — all loaded via layout <link>, all have Greek ── */
const FONTS = [
...
/* ── Main page ───────────────────────────────────────────────── */
export default function MiniWebCreatePage() {
  ...
  const bgStyle = bgStyleId === 'custom'
    ...
    try {
      ...
      await adminApi(token).post('/admin/invitations', {
        ...
        contacts: contacts.filter((c) => c.name).map((c) => ({
          ...c, phone: c.phone || undefined, email: c.email || undefined,
        })),
        giftRegistries: gifts.filter((g) => g.ownerName && g.iban).map((g) => ({
          ...g, bankName: g.bankName || undefined,
        })),
      });
      router.push('/admin');
    } catch (err: any) {
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex gap-8 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-6">
            ...
            {/* STYLE section — first, most exciting */}
              ...
              <div>
                  ...
                  {FONTS.map((f) => (
                    <button
                      ...
                      className={`relative flex flex-col items-start p-3 rounded-xl border transition-all text-left ${
                        fontFamily === f.name
                          ? 'border-[#01FFFF]/60 bg-[#01FFFF]/8'
                          : 'border-white/8 bg-[#07141C]/50 hover:border-white/20'
                      }`}
                    >
                      {fontFamily === f.name && (
                ...
                <div className="mt-2">
                  {customFontUrl ? (
                    <button
                      type="button"
                      onClick={() => setFontFamily('CustomWeddingFont')}
                    ...
                    >
                      ...
                      <span
                        role="button"
                        onClick={(e) => { e.stopPropagation(); fontInputRef.current?.click(); }}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {PALETTES.map((p) => (
                    ...
                    >
                      <div
                        className="w-8 h-8 rounded-full shadow-lg ring-2 ring-offset-2 ring-offset-[#071218] transition-all"
                  ...
                  >
                    <div
                      className="w-8 h-8 rounded-full shadow-lg ring-2 ring-offset-2 ring-offset-[#071218] overflow-hidden"
                      style={{
                        background: paletteId === 'custom'
                          ? customColor
                ...
                {paletteId === 'custom' && (
                  <div className="mt-3 p-4 bg-[#07141C]/80 rounded-xl border border-[#01FFFF]/15 space-y-3">
                    <div className="flex items-start gap-4">
                      {/* Colour swatch — click opens OS native picker */}
                      <div className="flex flex-col items-center gap-1.5 shrink-0">
                        <div
                          className="w-14 h-14 rounded-xl cursor-pointer border border-white/15 hover:border-white/30 transition-colors shadow-lg"
                        ...
                        <input ref={colorInputRef} type="color"
                          onChange={(e) => {
                            ...
                            setCustomHexInput(e.target.value.slice(1).toUpperCase());
                          }} className="sr-only" />
                      </div>
                            ...
                            <div key={ch} className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setCustomColor(hex);
                                  setCustomHexInput(hex.slice(1).toUpperCase());
                                }}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  ...
                  >
                    ...
                    <div
                      className="w-14 h-10 rounded-lg shadow-md"
                ...
                {/* Custom background gradient picker */}
                {bgStyleId === 'custom' && (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {([
                        ...
                        <div key={label} className="space-y-2">
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              className="w-10 h-10 rounded-lg cursor-pointer border border-white/15 hover:border-white/30 transition-colors shadow-md shrink-0"
