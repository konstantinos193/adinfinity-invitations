...
export default function FloatingBar({ events, gifts }: Props) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 bg-white/90 backdrop-blur-md rounded-full px-3 py-2 shadow-xl border border-[#b8960c]/20">
        {buttons.map((btn) => (
          <button
            key={btn.key}
            onClick={btn.onClick}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-medium transition-all ${
              btn.primary
                ? 'bg-[#2c1810] text-white hover:bg-[#5c3320]'
                : 'text-[#5c3320] hover:bg-[#f5efe6]'
            }`}
          >
            {btn.icon}
            <span>{btn.label}</span>
