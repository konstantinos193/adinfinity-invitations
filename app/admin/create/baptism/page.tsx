...
type ContactRole = 'FATHER' | 'MOTHER' | 'GODFATHER' | 'GODMOTHER';
...
const PALETTES = [
  ...
  { id: 'mint',      label: 'Μέντα',        primary: '#6ee7b7', dark: '#34d399', swatch: '#6ee7b7' },
  { id: 'lavender',  label: 'Λεβάντα',     primary: '#c4b5fd', dark: '#a78bfa', swatch: '#c4b5fd' },
  { id: 'peach',     label: 'Ροδακινί',    primary: '#fca5a5', dark: '#f87171', swatch: '#fca5a5' },
  { id: 'gold',      label: 'Χρυσό',       primary: '#b8960c', dark: '#8a6e08', swatch: '#b8960c' },
  { id: 'rose',      label: 'Rose',        primary: '#b76e79', dark: '#8a4a55', swatch: '#b76e79' },
...
const BG_STYLES = [
  { id: 'baby-blue',  label: 'Baby Blue',   gradient: 'linear-gradient(135deg,#0c2a3d 0%,#1a4f70 50%,#2d7db5 100%)', dot: '#2d7db5' },
  { id: 'pastel-pink',label: 'Pastel Pink', gradient: 'linear-gradient(135deg,#3d0d1e 0%,#7a2a44 50%,#b86080 100%)', dot: '#b86080' },
  { id: 'mint-fresh', label: 'Mint Fresh',  gradient: 'linear-gradient(135deg,#0d1e10 0%,#1e4022 50%,#386840 100%)', dot: '#386840' },
  { id: 'lavender-dream', label: 'Lavender', gradient: 'linear-gradient(135deg,#1a0a2e 0%,#3d1a6b 50%,#6b3aad 100%)', dot: '#6b3aad' },
    ...
    <div className="rounded-2xl overflow-hidden shadow-2xl border border-white/10">
      <div className="relative flex flex-col items-center justify-center py-12 px-6 text-center" style={{ background: bgStyle.gradient }}>
        ...
        {(fatherName || motherName) && (
          <p className="relative mt-3 text-white/60 text-xs">
      ...
      <div className="bg-[#fdfaf6] py-4 px-6 text-center">
        <p className="text-[10px] text-[#5c3320]/50 uppercase tracking-widest">Baptism Preview</p>
      </div>
    </div>
  );
}
...
export default function BaptismCreatePage() {
  ...
  const [customHexInput, setCustomHexInput] = useState('38BDF8');
  ...
  const [customBgFrom, setCustomBgFrom] = useState('#0c2a3d');
  const [customBgTo, setCustomBgTo] = useState('#2d7db5');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!slug || !childName || !eventDate) { setError('Συμπληρώστε τα υποχρεωτικά πεδία.'); return; }
    setLoading(true); setError('');
    try {
      const token = localStorage.getItem('admin_token');
      if (!token) { router.replace('/admin/login'); return; }
      const payload: Record<string, unknown> = {
        slug, childName, fatherName: fatherName || undefined, motherName: motherName || undefined,
        weddingDate: new Date(eventDate).toISOString(),
        story: story || undefined,
        rsvpDeadline: rsvpDeadline ? new Date(rsvpDeadline).toISOString() : undefined,
        status,
        eventCategory: 'BAPTISM',
        invitationType,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <Baby size={16} className="text-sky-400/70" />
        <h1 className="text-white font-semibold">Νέα Πρόσκληση Βάπτισης</h1>
        <span className="text-white/20 text-sm ml-auto">Βάπτιση</span>
      </div>
          ...
          <div className="flex-1 min-w-0 space-y-6">
            ...
            {invitationType === 'MINI_WEBSITE' && (
              <section className={sec}>
                ...
                <div>
                  <label className={lbl}>Γραμματοσειρά ονόματος</label>
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {FONTS.map((f) => (
                      <button key={f.name} type="button" onClick={() => setFontFamily(f.name)}
                        ...
                        <span className="text-white text-2xl leading-tight mb-1" style={{ fontFamily: `'${f.name}', serif` }}>Ελπίδα</span>
                  ...
                  <div className="mt-2">
                    {customFontUrl ? (
                      <button type="button" onClick={() => setFontFamily('CustomWeddingFont')}
                        ...
                        <span className="text-white text-2xl italic" style={{ fontFamily: 'CustomWeddingFont, serif' }}>Βάπτιση</span>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {[{ hex: '#ffffff', label: 'Λευκό' }, { hex: '#f5efe6', label: 'Κρεμ' }, { hex: '#38bdf8', label: 'Γαλάζιο' }, { hex: '#c4b5fd', label: 'Μοβ' }, { hex: '#6ee7b7', label: 'Μέντα' }, { hex: '#1a1a1a', label: 'Μαύρο' }].map(({ hex, label }) => (
            ...
            <section className={sec}>
              ...
              <div>
                ...
                <input className={inp} value={childName}
                  onChange={(e) => { setChildName(e.target.value); if (!slug) setSlug(autoSlug(e.target.value)); }}
                  placeholder="π.χ. Ελπίδα, Κωνσταντίνος" required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={lbl}>Ημερομηνία βάπτισης *</label>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input className={inp} value={slug}
                    ...
                    placeholder="elpida-baptism" required />
                </div>
              </div>
              <div>
              ...
              {invitationType === 'MINI_WEBSITE' ? (
                <>
                  ...
                  <div>
                    <label className={lbl}>Περιγραφή / ιστορία</label>
                    <textarea className={`${inp} resize-none`} rows={4} value={story} onChange={(e) => setStory(e.target.value)} placeholder="Λίγα λόγια για τη βάπτιση..." />
                  </div>
                </>
              ) : (
                <div>
                ...
                <div key={i} className="border border-[#01FFFF]/10 rounded-xl p-4 space-y-3 bg-[#07141C]/50">
                  <div className="flex items-center justify-between">
                    <select className="text-xs font-medium text-[#01FFFF]/80 bg-transparent border-none outline-none cursor-pointer" value={ev.type} onChange={(e) => upEv(i, 'type', e.target.value)}>
                      <option value="CEREMONY">Εκκλησία (Βάπτιση)</option>
            ...
            <section className={sec}>
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setContacts((p) => [...p, { role: 'GODFATHER', name: '', phone: '', email: '' }])}
                  className="flex items-center gap-1 text-xs text-[#01FFFF]/60 hover:text-[#01FFFF] transition-colors">
                  <Plus size={14} /> Προσθήκη
                </button>
              </div>
            ...
            <div className="flex gap-3 pb-12">
              ...
              <button type="submit" disabled={loading}
                className="flex-1 py-3 rounded-xl bg-sky-500 text-white text-sm font-bold hover:bg-sky-400 transition-colors disabled:opacity-50">
                {loading ? 'Αποθήκευση...' : '✓ Δημιουργία Βάπτισης'}
              </button>
            </div>
          </div>
