...
import { Plus, Trash2, ArrowLeft, Sparkles, Check, Upload, Loader2, Globe, Film, Video } from 'lucide-react';
...
const PALETTES = [
  ...
  { id: 'violet',    label: 'Βιολετί',     primary: '#8b5cf6', dark: '#6d28d9', swatch: '#8b5cf6' },
  { id: 'sky',       label: 'Γαλάζιο',     primary: '#38bdf8', dark: '#0ea5e9', swatch: '#38bdf8' },
...
const BG_STYLES = [
  ...
  { id: 'violet',     label: 'Violet',      gradient: 'linear-gradient(135deg,#1a0a2e 0%,#3d1a6b 50%,#6b3aad 100%)', dot: '#6b3aad' },
...
function PreviewCard({ brideName, groomName, childName, eventDate, fontFamily, palette, bgStyle }: {
  brideName: string; groomName: string; childName: string; eventDate: string;
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-2xl border border-white/10">
      <div className="relative flex flex-col items-center justify-center py-10 px-6 text-center" style={{ background: bgStyle.gradient }}>
        ...
        <p className="relative text-white/60 text-[10px] tracking-[0.3em] uppercase mb-3">Γάμος & Βάπτιση</p>
        <h2 className="relative text-white text-3xl italic leading-tight" style={{ fontFamily: `'${effectiveFont}', serif` }}>
          {brideName || 'Νύφη'} & {groomName || 'Γαμπρός'}
        </h2>
        {childName && (
          <p className="relative mt-2 text-white/70 text-sm">
            Βάπτιση: <span style={{ fontFamily: `'${effectiveFont}', serif` }}>{childName}</span>
          </p>
        )}
        <div className="relative mt-3 w-24 h-px" style={{ backgroundColor: palette.primary, opacity: 0.8 }} />
      ...
      <div className="bg-[#fdfaf6] py-3 px-6 text-center">
        <p className="text-[10px] text-[#5c3320]/50 uppercase tracking-widest">Wedding + Baptism Preview</p>
      </div>
    </div>
  );
}
...
export default function WeddingBaptismCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!slug || !brideName || !groomName || !childName || !eventDate) { setError('Συμπληρώστε τα υποχρεωτικά πεδία.'); return; }
    setLoading(true); setError('');
    try {
      const token = localStorage.getItem('admin_token');
      if (!token) { router.replace('/admin/login'); return; }
      const payload: Record<string, unknown> = {
        slug, brideName, groomName, childName,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-white font-semibold">Νέα Πρόσκληση Γαμοβάπτισης</h1>
        <span className="text-white/20 text-sm ml-auto">Γαμοβάπτιση</span>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex gap-8 items-start">
          <div className="flex-1 min-w-0 space-y-6">
            ...
            {invitationType === 'MINI_WEBSITE' && (
              <section className={sec}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {FONTS.map((f) => (
                      <button key={f.name} type="button" onClick={() => setFontFamily(f.name)}
                        className={`relative flex flex-col items-start p-3 rounded-xl border transition-all text-left ${fontFamily === f.name ? 'border-[#01FFFF]/60 bg-[#01FFFF]/8' : 'border-white/8 bg-[#07141C]/50 hover:border-white/20'}`}>
                        {fontFamily === f.name && (
                          <span className="absolute top-2 right-2 w-4 h-4 rounded-full bg-[#01FFFF] flex items-center justify-center">
                            <Check size={10} className="text-[#07141C]" />
                          </span>
                  ...
                  <div className="mt-2">
                    {customFontUrl ? (
                      <button type="button" onClick={() => setFontFamily('CustomWeddingFont')}
                        className={`relative w-full flex items-center gap-4 p-3 rounded-xl border transition-all text-left ${fontFamily === 'CustomWeddingFont' ? 'border-[#01FFFF]/60 bg-[#01FFFF]/8' : 'border-white/8 bg-[#07141C]/50 hover:border-white/20'}`}>
                        ...
                        <div className="flex-1 min-w-0"><div className="text-white/40 text-[10px] uppercase tracking-wider">Custom Upload</div></div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {PALETTES.map((p) => (
                      <button key={p.id} type="button" onClick={() => setPaletteId(p.id)} title={p.label}
                        className={`group relative flex flex-col items-center gap-1.5 p-2.5 rounded-xl border transition-all ${paletteId === p.id ? 'border-[#01FFFF]/60 bg-[#01FFFF]/5' : 'border-white/8 hover:border-white/20'}`}>
                    ...
                    <button type="button" onClick={() => setPaletteId('custom')} title="Custom"
                      className={`group relative flex flex-col items-center gap-1.5 p-2.5 rounded-xl border transition-all ${paletteId === 'custom' ? 'border-[#01FFFF]/60 bg-[#01FFFF]/5' : 'border-white/8 hover:border-white/20'}`}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {[{ hex: '#ffffff', label: 'Λευκό' }, { hex: '#f5efe6', label: 'Κρεμ' }, { hex: '#b8960c', label: 'Χρυσό' }, { hex: '#c4b5fd', label: 'Μοβ' }, { hex: '#fde68a', label: 'Κίτρινο' }, { hex: '#1a1a1a', label: 'Μαύρο' }].map(({ hex, label }) => (
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {([{ label: 'Χρώμα αρχής', color: customBgFrom, setColor: setCustomBgFrom, ref: bgFromRef }, { label: 'Χρώμα τέλους', color: customBgTo, setColor: setCustomBgTo, ref: bgToRef }] as const).map(({ label, color, setColor, ref }) => (
                          <div key={label} className="space-y-2">
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-10 h-10 rounded-lg cursor-pointer border border-white/15 hover:border-white/30 shadow-md shrink-0"
            ...
            {/* Core fields */}
            <section className={sec}>
              <SecTitle title="Στοιχεία ζευγαριού" />
              <div className="grid grid-cols-2 gap-4">
                <div>
            ...
            <section className={sec}>
              <SecTitle title="Στοιχεία βάπτισης" />
              <div>
                <label className={lbl}>Όνομα παιδιού *</label>
                <input className={inp} value={childName} onChange={(e) => setChildName(e.target.value)} placeholder="π.χ. Ελπίδα, Κωνσταντίνος" required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
            ...
            <section className={sec}>
              <SecTitle title="Ημερομηνία & Λεπτομέρειες" />
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={lbl}>Ημερομηνία εκδήλωσης *</label>
                  <input type="datetime-local" className={inp} value={eventDate} onChange={(e) => setEventDate(e.target.value)} required />
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input className={inp} value={slug}
                    ...
                    placeholder="ioanna-alexandros-elpida" required />
                </div>
              </div>
              <div>
              ...
              {invitationType === 'MINI_WEBSITE' ? (
                <>
                  ...
                  <div>
                    <label className={lbl}>Ιστορία / περιγραφή</label>
                    <textarea className={`${inp} resize-none`} rows={4} value={story} onChange={(e) => setStory(e.target.value)} placeholder="Η ιστορία του ζευγαριού και της βάπτισης..." />
                  </div>
                </>
              ) : (
                <div>
            ...
            {invitationType === 'MINI_WEBSITE' && (
              <section className={sec}>
                <SecTitle title="Gallery φωτογραφιών" />
                ...
                <div key={i} className="border border-[#01FFFF]/10 rounded-xl p-4 space-y-3 bg-[#07141C]/50">
                  <div className="flex items-center justify-between">
                    <select className="text-xs font-medium text-[#01FFFF]/80 bg-transparent border-none outline-none cursor-pointer" value={ev.type} onChange={(e) => upEv(i, 'type', e.target.value)}>
                      <option value="CEREMONY">Μυστήριο / Βάπτιση</option>
            ...
            <section className={sec}>
              <div className="flex items-center justify-between">
                <SecTitle title="Δωροδήλωση (IBAN)" />
              ...
              {gifts.map((g, i) => (
                <div key={i} className="border border-[#01FFFF]/10 rounded-xl p-4 space-y-3 bg-[#07141C]/50">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div><label className={lbl}>Ονοματεπώνυμο</label><input className={inp} value={g.ownerName} onChange={(e) => upGi(i, 'ownerName', e.target.value)} /></div>
            ...
            <div className="flex gap-3 pb-12">
              ...
              <button type="submit" disabled={loading}
                className="flex-1 py-3 rounded-xl bg-violet-600 text-white text-sm font-bold hover:bg-violet-500 transition-colors disabled:opacity-50">
                {loading ? 'Αποθήκευση...' : '✓ Δημιουργία Γαμοβάπτισης'}
              </button>
            </div>
          </div>
          ...
          {invitationType === 'MINI_WEBSITE' && (
            <div className="w-72 shrink-0 sticky top-8 hidden xl:block space-y-4">
              ...
              <PreviewCard brideName={brideName} groomName={groomName} childName={childName} eventDate={eventDate} fontFamily={fontFamily} palette={resolvedPalette} bgStyle={bgStyle} />
              <div className="bg-[#071218]/60 rounded-2xl p-4 border border-[#01FFFF]/10 space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-white/70" style={{ fontFamily: `'${fontFamily}', serif` }}>
